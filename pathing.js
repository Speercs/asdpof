	...
	* getAdjacentPositions(pos) {
	...
	findPath(startPos, targetPos, defaultOptions = {}) {
		...
		if (
			...
			this.getRoomDistance(startPos, targetPos) >= 1
		) {
	...
	makeStructuresMatrix(room, options) {
			...
			room.find(FIND_STRUCTURES).forEach( structure => {
				...
				if (structure.structureType === STRUCTURE_ROAD) {
					...
					!ignoreStructures && (
						...
						(!structure.my )
						// (!structure.my && !structure.isPublic) // original
					)
				) {
					matrix.setFast(x, y, 255);
				}
		...
		//}
			...
			// asdpof specific
			if( room.memory.base_x && room.memory.base_y ){
					// center
			    	matrix.setFast(room.memory.base_x+1, room.memory.base_y+1, 100);
					matrix.setFast(room.memory.base_x-1, room.memory.base_y+1, 100);
					matrix.setFast(room.memory.base_x-1, room.memory.base_y-1, 100);

					if( room.memory.h1_x && room.memory.h1_y ){
							if( room.memory.h1_type == 'h' ){
									matrix.setFast(room.memory.h1_x+1, room.memory.h1_y+0, 50);
									matrix.setFast(room.memory.h1_x-1, room.memory.h1_y+0, 50);
									matrix.setFast(room.memory.h1_x-0, room.memory.h1_y+0, 25);
							}
							else{
									matrix.setFast(room.memory.h1_x+0, room.memory.h1_y+1, 50);
									matrix.setFast(room.memory.h1_x+0, room.memory.h1_y-1, 50);
									matrix.setFast(room.memory.h1_x-0, room.memory.h1_y+0, 25);
							}
					}

					if( room.memory.h2_x && room.memory.h2_y ){
							if( room.memory.h2_type == 'h' ){
									matrix.setFast(room.memory.h2_x+1, room.memory.h2_y+0, 50);
									matrix.setFast(room.memory.h2_x-1, room.memory.h2_y+0, 50);
									matrix.setFast(room.memory.h2_x-0, room.memory.h2_y+0, 25);
							}
							else{
									matrix.setFast(room.memory.h2_x+0, room.memory.h2_y+1, 50);
									matrix.setFast(room.memory.h2_x+0, room.memory.h2_y-1, 50);
									matrix.setFast(room.memory.h2_x-0, room.memory.h2_y+0, 25);
							}
					}
					...
					var objs = _.filter(Game.creeps, (creep) => creep.memory.birth == room.name && 
					...
					if( objs && objs.length > 0 ){
							for ( var i = 0 ; i < objs.length ; i++){
									matrix.setFast( objs[i].pos.x, objs[i].pos.y, matrix.get( objs[i].pos.x, objs[i].pos.y ) + 15 );
							}
					}
					//
					...
					if( !room.storage ){
							const terrain2 = new Room.Terrain(room.name);
							// source
							if( room.memory.intel && room.memory.intel.sources ){
									if( room.memory.intel.sources[0] && room.memory.intel.sources[0].id ){
											var xx = Game.getObjectById( room.memory.intel.sources[0].id ).pos.x
											var yy = Game.getObjectById( room.memory.intel.sources[0].id ).pos.y
											for ( var xxx = -1 ; xxx <= 1 ; xxx++){
													for ( var yyy = -1 ; yyy <= 1 ; yyy++){
															if( terrain2.get(xx+xxx, yy+yyy) != TERRAIN_MASK_WALL ){
																	matrix.setFast(xxx+xx,yyy+yy, matrix.get(xxx+xx,yyy+yy) + 10 );
															}
													}
											}
									}
									if( room.memory.intel.sources[1] && room.memory.intel.sources[1].id ){
											var xx = Game.getObjectById( room.memory.intel.sources[1].id ).pos.x
											var yy = Game.getObjectById( room.memory.intel.sources[1].id ).pos.y
											for ( var xxx = -1 ; xxx <= 1 ; xxx++){
													for ( var yyy = -1 ; yyy <= 1 ; yyy++){
															if( terrain2.get(xx+xxx, yy+yyy) != TERRAIN_MASK_WALL ){
																	matrix.setFast(xxx+xx,yyy+yy, matrix.get(xxx+xx,yyy+yy) + 10 );
			...
			if( global.rooms[rm] && global.rooms[rm].path_to_avoid ){
				
				for(var cp in global.rooms[rm].path_to_avoid) {

					if( Game.creeps[cp] ){
						var xx = global.rooms[rm].path_to_avoid[cp][0]
						var yy = global.rooms[rm].path_to_avoid[cp][1]
						matrix.setFast(xx,yy, matrix.get(xxx+xx,yyy+yy) + 25 );	
					}
					else{
						delete global.rooms[rm].path_to_avoid[cp]
					}
